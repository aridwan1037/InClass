...
string[] honda = { "beat", "supra", "scoopy" };
string[] yamaha = { "mio", "nmax", "aerox" };
string[] suzuki = { "satria", "gsx" };

Dictionary<string, string[]> bikeBrand = new();
bikeBrand.Add("honda", honda);
bikeBrand.Add("yamaha", yamaha);
bikeBrand.Add("suzuki", suzuki);

foreach (var bike in bikeBrand)
{
    foreach (var myString in bike.Value)
    {
        Console.WriteLine(bike.Key + " : " + myString);
    }
}
...
// 			hashtable.Add("Email", "mailto:yes@yo.com");
