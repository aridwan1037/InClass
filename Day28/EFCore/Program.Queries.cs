...
partial class Program
{
    static void QueryingCategories()
    {
        using (Northwind db = new())
        {
            ...
            {
                Printer($"Category: {c.CategoryId}, {c.CategoryName} has {c.Products.Count} products.");
            }
        }
    }
    ...
    static void FilteredIncludes()
    {
        using (Northwind db = new())
        {
            ...
            if (!categories.Any(c => c.Products.Any(p => p.Stock >= stock)))
            {
                Fail("No Categories and product with minimum stock");
                return;
            }
            Info($"ToQueryString: {categories.ToQueryString()}");
